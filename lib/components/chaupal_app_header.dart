...
class ChaupalAppHeader extends StatelessWidget implements PreferredSizeWidget {
  const ChaupalAppHeader({
    ...
    required this.title,
    this.showBack = true,
    this.showWorkerLocation = false,
    this.onWorkerLocationChanged,
  });
  ...
  Widget build(BuildContext context) {
    ...
    return AppBar(
      ...
      leading: showBack ? _HeaderAction(icon: Icons.arrow_back_rounded, label: 'पीछे जाएँ', backgroundColor: const Color(0xFF1976D2), onTap: () { if (context.canPop()) context.pop(); }) : null,
