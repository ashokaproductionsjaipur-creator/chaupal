...
class _CreateJobPostWidgetState extends State<CreateJobPostWidget> {
  ...
  Future<void> _loadMasters() async {
    try {
      ...
      if (!mounted) return;
      setState(() {
        locationId = user?['chaupal_location_id'];
        professions = List<Map<String, dynamic>>.from(p as List);
      });
    } catch (_) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Profession/Chaupal data load नहीं हुआ.')),
        );
      }
    }
  }
  ...
  Future<void> _startRecording() async {
    ...
    timer = Timer.periodic(const Duration(seconds: 1), (t) async {
      if (!mounted) return;
      setState(() => seconds++);
      if (seconds >= 60) await _stopRecording();
    });
  }
  ...
  Future<void> _playRecording() async {
    if (audioPath != null) await player.play(DeviceFileSource(audioPath!));
  }
  ...
  Future<void> _submit() async {
    ...
    if (title.text.trim().isEmpty ||
        ...
        locationId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('सभी required fields भरें. Work Photo required है; Audio Note optional है.')),
      );
      return;
    }
    ...
    try {
      ...
      if (audioPath != null) {
        audioStoragePath = '$uid/job_$stamp.m4a';
        await SupaFlow.client.storage.from('job-media').upload(
          audioStoragePath!,
          File(audioPath!),
      ...
      // Prevent orphaned private media when backend validation/upload fails.
      try {
        if (photoPath != null) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              s.contains('posting_window_closed')
                  ? 'अभी Job Posting बंद है। अगली Posting window में try करें.'
                  : 'Job post नहीं हुई. Please try again.',
            ),
          ),
        );
      }
      ...
      body: ListView(
        ...
        children: [
          ...
          DropdownButtonFormField<int>(
            value: professionId,
            decoration: const InputDecoration(labelText: 'Job Profession | प्रोफेशन'),
            items: professions
          ...
          Card(
            ...
            child: Padding(
              ...
              child: Column(
                children: [
                  Row(children: [
                    const Expanded(child: Text('Audio Note | काम की जानकारी (optional, max 1 min)')),
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      IconButton(
                        onPressed: audioPath == null || recording ? null : _playRecording,
                        icon: const Icon(Icons.play_arrow_outlined, size: 32),
                      ),
                      IconButton(
                        onPressed: audioPath == null || recording ? null : _startRecording,
                        icon: const Icon(Icons.refresh_outlined, size: 28),
                      ),
                    ],
                  ),
                  if (audioPath != null) const Text('Audio ready ✓'),
                ],
              ),
            ),
          ),
          const SizedBox(height: 14),
          _field('Additional Details | अतिरिक्त जानकारी', details, 'Optional', maxLines: 4),
          const SizedBox(height: 14),
          _field('Expected Amount | अनुमानित राशि', amount, '₹ Amount', keyboard: TextInputType.number),
          const SizedBox(height: 14),
          ListTile(
            shape: RoundedRectangleBorder(
              borderRadius: BorderRadius.circular(10),
