...
class _WorkerRegistrationWidgetState extends State<WorkerRegistrationWidget> {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Dropdown data load failed: ${e.toString().replaceFirst('Exception: ', '')}')),
        );
      }
    } finally {
  ...
  Future<void> _register() async {
    ...
    if (name.text.trim().isEmpty ||
        ...
        locationId == null ||
        profilePhoto == null ||
        aadhaar.text.trim().length != 12 ||
        aadhaarPhoto == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Required fields सही भरें और सभी जरूरी photos चुनें. Password कम से कम 8 characters होना चाहिए.')),
      );
      return;
    }
    ...
    try {
      ...
      if (access == null || refresh == null || uid == null) {
        throw Exception('Registration session could not be created.');
      }
      ...
      final expires = body['expires_at'] is num
          ...
          : DateTime.now().add(Duration(seconds: (body['expires_in'] ?? 3600) as int));
      ...
      await authManager.signIn(
        ...
        authUid: uid,
        userData: ChaupalAuthUserStruct.fromMap(userMap),
      );

      final stamp = DateTime.now().millisecondsSinceEpoch;
      final profilePath = '$uid/profile_$stamp.jpg';
      final aadhaarPath = '$uid/aadhaar_$stamp.jpg';
      await SupaFlow.client.storage.from('profile-media').uploadBinary(
        profilePath,
        await profilePhoto!.readAsBytes(),
        fileOptions: const FileOptions(contentType: 'image/jpeg', upsert: false),
      );
      await SupaFlow.client.storage.from('aadhaar-private').uploadBinary(
        aadhaarPath,
        await aadhaarPhoto!.readAsBytes(),
        fileOptions: const FileOptions(contentType: 'image/jpeg', upsert: false),
      );
      ...
      if (role == 'worker') {
        ...
        await SupaFlow.client.storage.from('worker-verification').uploadBinary(
          livePath,
          await livePhoto!.readAsBytes(),
          fileOptions: const FileOptions(contentType: 'image/jpeg', upsert: false),
        );
      }
      ...
      final result = await SupaFlow.client.rpc(
        ...
        params: {
          ...
          'p_other_profession': otherProfession.text.trim(),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(role == 'worker' ? 'Account created. Verification pending.' : 'Account created successfully.')),
        );
        if (role == 'worker') {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        title: const Text('Create Account | अकाउंट बनाएं'),
      ),
      body: ListView(
        ...
        children: [
          Row(
            children: [
              Expanded(child: ChoiceChip(label: const Text('Owner | मालिक'), selected: role == 'owner', onSelected: (_) => setState(() => role = 'owner'))),
              const SizedBox(width: 10),
              Expanded(child: ChoiceChip(label: const Text('Worker | कामगार'), selected: role == 'worker', onSelected: (_) => setState(() => role = 'worker'))),
            ],
          ),
          const SizedBox(height: 14),
          _field('Full Name | पूरा नाम', name, 'अपना नाम'),
          const SizedBox(height: 12),
          _field('Mobile Number | मोबाइल', mobile, '10 digit', keyboard: TextInputType.phone),
          const SizedBox(height: 12),
          _field('Username | यूजरनेम', username, 'username'),
          const SizedBox(height: 12),
          _field('Password | पासवर्ड', password, 'कम से कम 8 characters', obscure: true),
          const SizedBox(height: 12),
          _field('Confirm Password | पुष्टि', confirm, 'password again', obscure: true),
          const SizedBox(height: 12),
          _locationDropdown(),
          if (role == 'worker') ...[
            const SizedBox(height: 12),
            _professionDropdown(),
            const SizedBox(height: 12),
            _field('Other Profession | अन्य', otherProfession, 'Only if Other Profession'),
          ],
          const SizedBox(height: 12),
          _field('Aadhaar Number | आधार', aadhaar, '12 digit', keyboard: TextInputType.number),
          const SizedBox(height: 12),
          ...
          if (role == 'worker') ...[
            const SizedBox(height: 10),
            _photoTile('Live Verification Photo | Live फोटो *', livePhoto, () => _pickAndSet('live'), cameraOnly: true),
          ],
          ...
          SizedBox(
            ...
            child: FilledButton(
              ...
              child: Text(busy ? 'Creating...' : 'Create Account | अकाउंट बनाएं'),
            ),
          ),
        ],
      ),
          ...
          children: [
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  Text(file == null ? label : '$label ✓', style: const TextStyle(fontWeight: FontWeight.w600)),
                  const SizedBox(height: 4),
                  Text(file == null ? (cameraOnly ? 'Camera से photo लें' : 'Camera या Gallery से photo चुनें') : 'Photo selected • preview visible • Tap to change'),
                ],
              ),
            ),
  ...
  Widget _locationDropdown() {
    return DropdownButtonFormField<int>(
      ...
      items: loadingMasters ? const [] : locations.map((x) {
        final id = (x['id'] as num).toInt();
        return DropdownMenuItem<int>(value: id, child: Text('${x['display_name']}', overflow: TextOverflow.ellipsis));
      }).toList(),
  ...
  Widget _field(String label, TextEditingController c, String hint, {TextInputType? keyboard, bool obscure = false}) {
    return TextField(
      controller: c,
