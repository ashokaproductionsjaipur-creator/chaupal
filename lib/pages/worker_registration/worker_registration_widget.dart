...
class _WorkerRegistrationWidgetState extends State<WorkerRegistrationWidget> {
  ...
  @override
  void dispose() {
    for (final c in [name, mobile, username, password, confirm, aadhaar, otherProfession]) {
      c.dispose();
    }
    super.dispose();
  }
  ...
  Future<void> _loadMasters() async {
    ...
    try {
      ...
      if (r.statusCode < 200 || r.statusCode >= 300 || body is! Map || body['ok'] != true) {
        throw Exception(body is Map && body['message'] is String ? body['message'] : 'Master data could not be loaded.');
      }
      final rawLocations = body['locations'];
      final rawProfessions = body['professions'];
      if (mounted) {
        setState(() {
          locations = List<Map<String, dynamic>>.from(rawLocations is List ? rawLocations : const []);
          professions = List<Map<String, dynamic>>.from(rawProfessions is List ? rawProfessions : const []);
        });
      }
    } catch (e) {
  ...
  Future<void> _register() async {
    ...
    if (name.text.trim().isEmpty || !RegExp(r'^\d{10}$').hasMatch(m) || u.length < 3 || p.length < 8 || p != confirm.text || locationId == null || profilePhoto == null || aadhaar.text.trim().length != 12 || aadhaarPhoto == null) {
    ...
    try {
      final response = await http.post(
        ...
        body: jsonEncode({'action': 'signup', 'username': u, 'password': p, 'role': role, 'full_name': name.text.trim(), 'mobile_number': m}),
      );
      dynamic body;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        title: const Text('Create Account', style: TextStyle(fontWeight: FontWeight.w900)),
      ),
      body: Center(
          ...
          child: ListView(
            ...
            children: [
              Container(
                ...
                decoration: BoxDecoration(color: t.secondaryBackground, borderRadius: BorderRadius.circular(24), border: Border.all(color: t.alternate)),
                child: Row(children: [
                  ...
                  const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('Join CHAUPAL', style: TextStyle(fontSize: 19, fontWeight: FontWeight.w900)), SizedBox(height: 3), Text('काम और कामगार — एक ही जगह', style: TextStyle(fontSize: 12))])),
                  const Icon(Icons.verified_outlined),
                ]),
              ),
              const SizedBox(height: 18),
              Text('आप कैसे जुड़ना चाहते हैं?', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w900, color: t.primaryText)),
              const SizedBox(height: 4),
              Text('Choose your role to continue', style: TextStyle(color: t.secondaryText, fontSize: 13)),
              const SizedBox(height: 14),
              Row(children: [
                Expanded(child: _roleChoice(t, 'owner', Icons.business_center_rounded, 'Owner', 'मालिक')),
                const SizedBox(width: 10),
                Expanded(child: _roleChoice(t, 'worker', Icons.construction_rounded, 'Worker', 'कामगार')),
              ]),
              const SizedBox(height: 22),
              _section(t, 'Basic Information', 'बुनियादी जानकारी'),
              const SizedBox(height: 12),
              _field('Full Name | पूरा नाम', name, 'अपना नाम', icon: Icons.person_outline_rounded),
              const SizedBox(height: 11),
              _field('Mobile Number | मोबाइल', mobile, '10 digit mobile', keyboard: TextInputType.phone, icon: Icons.phone_outlined),
              const SizedBox(height: 11),
              _field('Username | यूजरनेम', username, 'unique username', icon: Icons.alternate_email_rounded),
              const SizedBox(height: 11),
              _field('Password | पासवर्ड', password, 'कम से कम 8 characters', obscure: true, icon: Icons.lock_outline_rounded),
              const SizedBox(height: 11),
              _field('Confirm Password | पुष्टि', confirm, 'password again', obscure: true, icon: Icons.lock_reset_outlined),
              const SizedBox(height: 18),
              _section(t, 'Work Details', 'काम की जानकारी'),
              const SizedBox(height: 12),
              _locationDropdown(),
              if (role == 'worker') ...[
                const SizedBox(height: 11), _professionDropdown(), const SizedBox(height: 11), _field('Other Profession | अन्य', otherProfession, 'Only if needed', icon: Icons.work_outline),
              ],
              const SizedBox(height: 18),
              _section(t, 'Verification', 'सत्यापन'),
              const SizedBox(height: 5),
              Text(role == 'worker' ? 'Worker verification के लिए identity और live photo जरूरी हैं.' : 'Identity verification के लिए Aadhaar और profile photo जरूरी हैं.', style: TextStyle(color: t.secondaryText, fontSize: 12, height: 1.35)),
              const SizedBox(height: 12),
              _field('Aadhaar Number | आधार', aadhaar, '12 digit', keyboard: TextInputType.number, icon: Icons.badge_outlined),
              const SizedBox(height: 11),
              _photoTile('Profile Photo | प्रोफाइल फोटो *', profilePhoto, () => _pickAndSet('profile'), cameraOnly: false),
              ...
              SizedBox(height: 56, child: FilledButton.icon(onPressed: busy ? null : _register, icon: Icon(busy ? Icons.hourglass_top_rounded : Icons.check_circle_outline_rounded), label: Text(busy ? 'Creating account...' : 'Create Account  |  अकाउंट बनाएं', style: const TextStyle(fontWeight: FontWeight.w800)))),
              const SizedBox(height: 14),
              Text('By continuing, you agree to use CHAUPAL responsibly.', textAlign: TextAlign.center, style: TextStyle(color: t.secondaryText, fontSize: 10.5)),
            ],
          ),
        ),
      ),
  ...
  Widget _roleChoice(FlutterFlowTheme t, String value, IconData icon, String en, String hi) {
    ...
    return InkWell(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(color: selected ? t.primaryText.withValues(alpha: 0.06) : t.secondaryBackground, borderRadius: BorderRadius.circular(18), border: Border.all(color: selected ? t.primaryText : t.alternate, width: selected ? 1.5 : 1)),
        child: Row(children: [
          Container(width: 43, height: 43, decoration: BoxDecoration(color: selected ? t.primaryText : t.primaryText.withValues(alpha: 0.07), borderRadius: BorderRadius.circular(14)), child: Icon(icon, color: selected ? t.primaryBackground : t.primaryText)),
          const SizedBox(width: 10),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(en, style: const TextStyle(fontWeight: FontWeight.w800)), Text(hi, style: TextStyle(color: t.secondaryText, fontSize: 11))])),
          Icon(selected ? Icons.check_circle_rounded : Icons.circle_outlined, size: 21),
        ]),
      ),
    );
  }
  ...
  Widget _photoTile(String label, XFile? file, VoidCallback onTap, {required bool cameraOnly}) {
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(color: t.secondaryBackground, borderRadius: BorderRadius.circular(18), border: Border.all(color: t.alternate)),
        child: Row(children: [
          _LocalPhotoPreview(file: file, size: 72),
          const SizedBox(width: 13),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(file == null ? label : '$label ✓', style: const TextStyle(fontWeight: FontWeight.w700)), const SizedBox(height: 4), Text(file == null ? (cameraOnly ? 'Camera से photo लें' : 'Camera या Gallery से photo चुनें') : 'Photo selected • Tap to change', style: TextStyle(color: t.secondaryText, fontSize: 11.5))])),
          Icon(file == null ? Icons.add_a_photo_outlined : Icons.edit_outlined),
        ]),
      ),
    );
  }

  Widget _locationDropdown() => DropdownButtonFormField<int>(
    value: locationId,
    isExpanded: true,
    menuMaxHeight: 420,
    decoration: const InputDecoration(labelText: 'Chaupal Location | चौपाल लोकेशन', hintText: 'Select Chaupal Location', border: OutlineInputBorder(), suffixIcon: Icon(Icons.location_on_outlined)),
    items: loadingMasters ? const [] : locations.map((x) { final id = (x['id'] as num).toInt(); return DropdownMenuItem<int>(value: id, child: Text('${x['display_name']}', overflow: TextOverflow.ellipsis)); }).toList(),
    onChanged: loadingMasters ? null : (v) => setState(() => locationId = v),
  );

  Widget _professionDropdown() => DropdownButtonFormField<int>(
    value: professionId,
    isExpanded: true,
    menuMaxHeight: 420,
    decoration: const InputDecoration(labelText: 'Profession | प्रोफेशन', hintText: 'Select Profession', border: OutlineInputBorder(), suffixIcon: Icon(Icons.work_outline)),
    items: professions.map((x) { final id = (x['id'] as num).toInt(); return DropdownMenuItem<int>(value: id, child: Text('${x['display_name']}', overflow: TextOverflow.ellipsis)); }).toList(),
    onChanged: (v) => setState(() => professionId = v),
  );

  Widget _field(String label, TextEditingController c, String hint, {TextInputType? keyboard, bool obscure = false, IconData? icon}) => TextField(
    controller: c,
    keyboardType: keyboard,
    obscureText: obscure,
    decoration: InputDecoration(labelText: label, hintText: hint, border: const OutlineInputBorder(), prefixIcon: icon == null ? null : Icon(icon)),
  );
}
    ...
    return ClipRRect(
      ...
      child: FutureBuilder<Uint8List>(
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.done && snapshot.hasData) return Image.memory(snapshot.data!, width: size, height: size, fit: BoxFit.cover);
          return Container(width: size, height: size, decoration: BoxDecoration(borderRadius: BorderRadius.circular(12), border: Border.all(color: FlutterFlowTheme.of(context).alternate)), child: const Center(child: SizedBox(width: 22, height: 22, child: CircularProgressIndicator(strokeWidth: 2))));
        },
      ),
    );
  }
