...
class _AdminVerificationPanelWidgetState extends State<AdminVerificationPanelWidget> {
  ...
  Future<List<Map<String, dynamic>>> _load() async { final r = await SupaFlow.client.rpc('get_admin_pending_workers'); return List<Map<String, dynamic>>.from(r as List); }
  Future<void> _review(Map<String, dynamic> w, String status) async {
    try { await SupaFlow.client.rpc('review_worker_verification', params: {'p_worker_id': w['user_id'], 'p_status': status, 'p_note': status == 'more_info' ? 'Please provide clearer verification information.' : null}); if (mounted) { ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(status == 'approved' ? 'Worker approved.' : status == 'rejected' ? 'Worker rejected.' : 'More information requested.'))); setState(() => future = _load()); } } catch (_) { if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Admin action failed.'))); }
  }
  Future<void> _view(String bucket, String? path) async {
    if (path == null || path.isEmpty) return;
    try { final r = await SupaFlow.client.storage.from(bucket).createSignedUrl(path, 300); if (mounted) showDialog(context: context, builder: (c) => AlertDialog(title: const Text('Private Document'), content: SizedBox(width: 300, height: 360, child: Image.network(r, fit: BoxFit.contain, errorBuilder: (_, __, ___) => const Center(child: Text('Preview unavailable.')))), actions: [TextButton(onPressed: () => Navigator.pop(c), child: const Text('Close'))])); } catch (_) { if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Private document could not be opened.'))); }
  }
